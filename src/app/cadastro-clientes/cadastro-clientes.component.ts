...
export class CadastroClientesComponent {
  ...
  //declarar a inicializar o objeto da classe HttpClient
  ...
  form = new FormGroup({
    ...
    tipo: new FormControl('', [
      Validators.required,
      Validators.pattern(/^(Comum|Preferencial|VIP])$/),
    ]),
  });
  ...
  //função para executar quando o usuário clicar
  //no botão SUBMIT no formulário
  onSubmit() {
    this.httpClient.post(environment.apiClientes, this.form.value).subscribe({
      //aguardando a resposta da API^
      next: (data: any) => {
        //capturando a resposta obtida^
